...
# 5.1.А. Сброс контекста в Business-чате (ЗАКОММЕНТИРОВАНО!)
...
# 5.2. ТЕКСТ -> ТЕКСТ (С памятью, Business Chat) (ЗАКОММЕНТИРОВАНО!)
...
# 5.3. ГОЛОС -> ГОЛОС (С памятью, Business Chat) (ЗАКОММЕНТИРОВАНО!)
...
# 5.4. НЕОБРАБОТАННЫЕ СООБЩЕНИЯ В BUSINESS CHAT (стикеры, фото) (ЗАКОММЕНТИРОВАНО!)
