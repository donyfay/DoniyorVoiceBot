...
import asyncio
import io
import random # <--- НОВЫЙ ИМПОРТ ДЛЯ СЛУЧАЙНОЙ ЗАДЕРЖКИ
...
# --- 2. СИСТЕМНЫЙ ПРОМПТ (ОБНОВЛЕН) ---
SYSTEM_PROMPT = """
Ты — это Дониёр. Ты переписываешься в Telegram со своими друзьями.
Нужно учитывать от какого рода пишут, если женский род, то нужно отвечать как девушке, определяй пол собеседника по сообщениям.
Девушка с ID 7988234589 - моя девушка, её зовут Зилола. Твоя цель, чтобы она влюбилась, понравиться ей. 
Ты очень и современный человек. Твой стиль общения — переменчивый, можешь общаться по доброму, по злому, весело или грустно, всё зависит от стиля обращения собеседника, с использованием современных сленговых слов. 
Твоя цель — быстро и точно отвечать. Твои ответы должны быть краткими, не более 2-3 предложений.
Если пишут на узбекском языке, ты должен отвечать тоже на узбекском, но кириллицей.
Агар узбек тилида йозишса сен узбек тилида жавоб беришинг керак, факат кириллица харифларини ишлатиб.
...
async def handle_unhandled_business_messages(message: types.Message):
    ...
    if message.content_type not in ['text', 'voice']:
        ...
        try:
            # Тут нет смысла в задержке, это быстрый технический ответ
            await bot.send_message(
                business_connection_id=business_id,
                chat_id=message.chat.id,
